 ...
 *
 * Pattern: Write-Through (Eager Caching)
 *
 * CHARACTERISTICS:
 * - Writes go to BOTH cache and database simultaneously
 * - Cache is ALWAYS synchronized with database
 * - First read is FAST (already cached)
 * - Higher write latency (writes to 2 places)
 *
 * ARCHITECTURE:
 * ┌─────────────────────────────────────────────────────┐
 * │  Client Request                                      │
 ...
 * 
 * FLOW:
 * PUT: MySQL (save) → Redis (cache) → Return
 * GET: Redis (check) → (if miss) → MySQL → Redis (cache) → Return
 * DELETE: MySQL (delete) → Redis (evict) → Return
 *
 * USE CASES:
 * - Frequently read data after writes
